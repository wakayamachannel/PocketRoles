...
namespace PocketRoles.Core
{
    ...
    public sealed class RoleInfo
    {
        ...
        /// <summary>
        /// v0.5.5: Chinese names of earlier versions (v0.5.4 and the v0.5.5 drafts, before PR #1's names), still accepted as
        /// input by <see cref="Roles.TryParse"/>: whole (/cmd r 崇拜者, /set 内鬼狂粉 1) and as a prefix after the current names (疯, 鹰, 崇).
        /// Never shown.
        /// </summary>
        public string[] FormerZh = Array.Empty<string>();
    ...
    public static class Roles
    {
        ...
        public static readonly RoleInfo[] All =
        {
            new RoleInfo { Id = CustomRole.Sheriff, Key = "sheriff", NameJa = "シェリフ", NameEn = "Sheriff", Team = Team.Crew, Color = "#f8cd46",
                ...
                NameZh = "警长", DescZh = "可用击杀键射杀伪装者、豺狼和纵火犯（按设置也可射杀狂信徒系职业、跟班）。误杀其他人会让自己死亡。不能钻通风口和破坏，任务是假的。",
                Aliases = new[] { "sh" } },
            ...
            new RoleInfo { Id = CustomRole.Snitch, Key = "snitch", NameJa = "スニッチ", NameEn = "Snitch", Team = Team.Crew, Color = SnitchColor,
                TasksCount = true,
                DescJa = "タスクを全て終えるとインポスターの名前が赤く、ジャッカルが青く見えます。残りタスクが少なくなるとキラー側にはあなたが誰か分かります。",
                DescEn = "Finish all tasks to see Impostors in red, the Jackal in blue. When you are almost done, killers can see who you are.",
                NameZh = "告密者", DescZh = "完成全部任务后，伪装者的名字会显示为红色，豺狼为蓝色。任务快完成时，他们会知道你是谁。",
                Aliases = new[] { "sn" } },
            new RoleInfo { Id = CustomRole.Lighter, Key = "lighter", NameJa = "ライター", NameEn = "Lighter", Team = Team.Crew, Color = "#eee5be",
                ...
                NameZh = "执灯人", DescZh = "你的视野比普通船员更大。",
                Aliases = new[] { "lt" }, FormerZh = new[] { "点灯人" } },   // terms-ok: former name, input only
            new RoleInfo { Id = CustomRole.SpeedBooster, Key = "speedbooster", NameJa = "スピードブースター", NameEn = "Speed Booster", Team = Team.Crew, Color = "#00ffff",
                ...
                NameZh = "加速者", DescZh = "你的移动速度比普通船员更快。",
                Aliases = new[] { "sb", "speed", "booster" }, FormerZh = new[] { "增速者" } },   // terms-ok: former name, input only
            ...
            new RoleInfo { Id = CustomRole.Madmate, Key = "madmate", NameJa = "マッドメイト", NameEn = "Madmate", Team = Team.Impostor, Color = ImpostorColor,
                ...
                NameZh = "狂信徒", DescZh = "伪装者阵营的船员。伪装者的名字显示为红色，但你不能击杀。伪装者获胜时你也获胜。",
                Aliases = new[] { "mad", "mm" }, FormerZh = new[] { "伪装者狂粉", "内鬼狂粉" } },   // terms-ok: former names (v0.5.5 draft, v0.5.4), input only
            ...
            // `マッドメイ` stay Madmate (row above); the shortest unique prefixes are マッドメイヤ / マッドス / マッドホ / 崇拝 and 狂信徒市 / 狂信徒特 / 狂信徒鹰 / 传
            // (狂 / 狂信徒 = Madmate since PR #1's names; the former 疯 / 鹰 / 崇 still work through FormerZh)
            // (README alias sentence quotes them). DescEn of the Mad Mayor is ≤ 100 chars on purpose: /cmd r <role> is capped at 3 messages.
            new RoleInfo { Id = CustomRole.MadMayor, Key = "madmayor", NameJa = "マッドメイヤー", NameEn = "Mad Mayor", NameZh = "狂信徒市长", Team = Team.Impostor, Color = ImpostorColor,
                TasksCount = false,
                ...
                DescZh = "伪装者阵营的船员，会议中的投票按多票计算。伪装者的名字显示为红色，但你不能击杀。伪装者获胜时你也获胜。任务是假的。",
                Aliases = new[] { "mmy", "madmy", "mmayor" }, FormerZh = new[] { "狂粉市长" } },   // terms-ok: former name, input only
            new RoleInfo { Id = CustomRole.MadStuntman, Key = "madstuntman", NameJa = "マッドスタントマン", NameEn = "Mad Stuntman", NameZh = "狂信徒特技演员", Team = Team.Impostor, Color = ImpostorColor,
                ...
                DescZh = "伪装者阵营的船员。伪装者的名字显示为红色，你不能击杀，但前几次被击杀不会死（投票驱逐无法避免）。伪装者获胜时你也获胜。",
                Aliases = new[] { "stunt", "madstunt", "mst", "ms" }, FormerZh = new[] { "疯狂特技演员" } },   // terms-ok: former name, input only
            new RoleInfo { Id = CustomRole.MadHawk, Key = "madhawk", NameJa = "マッドホーク", NameEn = "Mad Hawk", NameZh = "狂信徒鹰眼", Team = Team.Impostor, Color = ImpostorColor,
                ...
                DescZh = "伪装者阵营的船员，视野比普通船员大得多。伪装者的名字显示为红色，但你不能击杀。伪装者获胜时你也获胜。",
                Aliases = new[] { "mh", "mhawk" }, FormerZh = new[] { "鹰眼狂粉" } },   // terms-ok: former name, input only
            // Madmate with a button: its own client is an Impostor (kill button = worship), everyone else sees a Crewmate.
            new RoleInfo { Id = CustomRole.Worshipper, Key = "worshipper", NameJa = "崇拝者", NameEn = "Worshipper", NameZh = "传教士", Team = Team.Impostor, Color = ImpostorColor,
                IsKiller = true, ImpostorDesync = true, CanVent = false, CanSabotage = false, TasksCount = false,
                ...
                DescZh = "伪装者阵营的船员（不知道谁是伪装者）。击杀键变为“传教”，对目标使用后对方成为狂信徒（次数有限）。传教给伪装者会自爆，次数用完后只会失败。不能钻通风口和破坏，任务是假的。",
                Aliases = new[] { "ws", "worship" }, FormerZh = new[] { "崇拜者" } },   // terms-ok: former name, input only
            new RoleInfo { Id = CustomRole.Vampire, Key = "vampire", NameJa = "ヴァンパイア", NameEn = "Vampire", Team = Team.Impostor, Color = ImpostorColor,
                ...
                NameZh = "吸血鬼", DescZh = "伪装者。你的击杀变成“咬人”，目标在几秒后死亡。",
                Aliases = new[] { "vamp", "vp" } },
            new RoleInfo { Id = CustomRole.Mafia, Key = "mafia", NameJa = "マフィア", NameEn = "Mafia", Team = Team.Impostor, Color = ImpostorColor,
                ...
                NameZh = "黑手党", DescZh = "伪装者。在其他伪装者全部死亡之前不能击杀。",
                Aliases = new[] { "mf" } },
            new RoleInfo { Id = CustomRole.Witch, Key = "witch", NameJa = "魔女", NameEn = "Witch", NameZh = "女巫", Team = Team.Impostor, Color = ImpostorColor,
                ...
                DescZh = "伪装者。你的击杀变成“诅咒”，下次会议结束后所有被诅咒的人一起死亡。你被驱逐或死亡则诅咒失效。",
                Aliases = new[] { "wt", "wi" } },
            new RoleInfo { Id = CustomRole.Assassin, Key = "assassin", NameJa = "アサシン", NameEn = "Assassin", NameZh = "刺客", Team = Team.Impostor, Color = ImpostorColor,
                ...
                DescZh = "伪装者。会议中输入 /cmd guess 名字 职业：猜对则对方死亡，猜错则你死亡。",
                Aliases = new[] { "as", "asn" } },
            ...
            new RoleInfo { Id = CustomRole.EvilHawk, Key = "evilhawk", NameJa = "イビルホーク", NameEn = "Evil Hawk", NameZh = "邪恶鹰眼", Team = Team.Impostor, Color = ImpostorColor,
                ...
                DescZh = "伪装者。视野比普通伪装者大得多。击杀、钻通风口、破坏与普通伪装者相同。",
                Aliases = new[] { "eh" } },
            new RoleInfo { Id = CustomRole.EvilNekomata, Key = "evilnekomata", NameJa = "イビル猫又", NameEn = "Evil Nekomata", NameZh = "邪恶猫又", Team = Team.Impostor, Color = ImpostorColor,
                ...
                DescZh = "伪装者，可以正常击杀。当你在会议中被投票驱逐时，会从投票给你的人中随机拖一人一起死。",
            ...
            new RoleInfo { Id = CustomRole.SerialKiller, Key = "serialkiller", NameJa = "シリアルキラー", NameEn = "Serial Killer", NameZh = "连环杀手", Team = Team.Impostor, Color = ImpostorColor,
                ...
                DescZh = "伪装者。击杀冷却很短，但若不在限定时间内击杀他人，就会当场死亡（每次击杀后重置，会议中暂停）。可钻通风口、可破坏。",
            ...
            new RoleInfo { Id = CustomRole.Samurai, Key = "samurai", NameJa = "侍", NameEn = "Samurai", NameZh = "武士", Team = Team.Impostor, Color = ImpostorColor,
                ...
                DescZh = "伪装者。你的击杀变成“斩击”：击杀目标之后，那一刻你周围的所有人也会接连死亡。冷却时间较长。",
                Aliases = new[] { "sam", "sm" } },
            new RoleInfo { Id = CustomRole.Jester, Key = "jester", NameJa = "ジェスター", NameEn = "Jester", Team = Team.Neutral, Color = "#ec62a5",
                ...
                NameZh = "小丑", DescZh = "独立阵营。在会议中被投票驱逐即单独获胜。任务是假的。",
                Aliases = new[] { "js" } },
            new RoleInfo { Id = CustomRole.Opportunist, Key = "opportunist", NameJa = "オポチュニスト", NameEn = "Opportunist", Team = Team.Neutral, Color = "#00ff00",
                ...
                NameZh = "投机主义者", DescZh = "独立阵营。游戏结束时仍存活即可与胜者一同获胜。任务是假的。",
                Aliases = new[] { "opp", "op" }, FormerZh = new[] { "投机者" } },   // terms-ok: former name, input only
            new RoleInfo { Id = CustomRole.Terrorist, Key = "terrorist", NameJa = "テロリスト", NameEn = "Terrorist", Team = Team.Neutral, Color = "#00ff00",
                ...
                NameZh = "恐怖分子", DescZh = "独立阵营。完成全部任务后被击杀或被投票驱逐即单独获胜。",
            ...
            new RoleInfo { Id = CustomRole.Jackal, Key = "jackal", NameJa = "ジャッカル", NameEn = "Jackal", Team = Team.Neutral, Color = JackalColor,
                ...
                NameZh = "豺狼", DescZh = "独立阵营的杀手。可以击杀任何人。消灭全部伪装者，且剩余船员人数不超过你时获胜。",
                Aliases = new[] { "jk" } },
            ...
            new RoleInfo { Id = CustomRole.JackalFriends, Key = "jackalfriends", NameJa = "ジャッカルフレンズ", NameEn = "Jackal Friends", NameZh = "跟班", Team = Team.Neutral, Color = JackalColor,
                TasksCount = false,
                ...
                Aliases = new[] { "jf", "friends", "friend" }, FormerZh = new[] { "豺狼之友" } },   // terms-ok: former name, input only
            ...
            new RoleInfo { Id = CustomRole.Arsonist, Key = "arsonist", NameJa = "放火魔", NameEn = "Arsonist", NameZh = "纵火犯", Team = Team.Neutral, Color = ArsonistColor,
                ...
                DescZh = "独立阵营。击杀键变成“浇油”，不会杀人。给所有其他存活玩家浇油后立即单独获胜。不能破坏，任务是假的。",
        ...
        public static bool TryParse(string text, out CustomRole role)
        {
            ...
            // Japanese / Chinese partial match (e.g. "シェリ", "警"). v0.5.5: never for a vanilla role word or the start of a
            // Chinese one: "伪" / "伪装" stay the Impostor, not the Madmate's former name 伪装者狂粉 (RoleWords.PrefixMatch).
            // The former Chinese names (RoleInfo.FormerZh) come after the current ones: 疯 / 鹰 / 崇 still work.
            int i = RoleWords.PrefixMatch(PrefixNamesJa, PrefixNamesZh, PrefixFormerZh, t);
            if (i >= 0) { role = All[i].Id; return true; }
            return false;
        }

        private static string[] _prefixJa, _prefixZh;
        private static string[][] _prefixFormerZh;
        private static string[] PrefixNamesJa => _prefixJa ?? (_prefixJa = Array.ConvertAll(All, r => r.NameJa));
        private static string[] PrefixNamesZh => _prefixZh ?? (_prefixZh = Array.ConvertAll(All, r => r.NameZh));
        private static string[][] PrefixFormerZh => _prefixFormerZh ?? (_prefixFormerZh = Array.ConvertAll(All, r => r.FormerZh));
        ...
        public static string TeamName(Team t)
        {
            switch (t)
            {
                case Team.Impostor: return Lang.T("team.impostor", "インポスター陣営", "Impostor team", "伪装者阵营");
                case Team.Neutral: return Lang.T("team.neutral", "第三陣営", "Neutral", "独立阵营");
