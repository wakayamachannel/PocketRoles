...
- 登録オフ（便利ホスト）の部屋では役職を配りません。全体メッセージでホストが切断される可能性が残っています。逆スケルド（Dleks）は登録オフの部屋では既定で出ません（選んだ直後にサーバーがホストを切断した実例があったため。`/opt lobby.dleks.compat on` で自己責任で出せます）。登録オフは設定に保存されるので、戻すときは `/opt register on` → 部屋を作り直し。
- 会議のあとの黒画面防止は、2026.8.18 では一部の状況（インポスターが全員死んでいて第三陣営だけが残っている、シェリフ等が死亡後に観戦している、など）で効かない可能性があります。黒画面になった参加者は部屋に戻ると復帰します。2 人だけの試合では会議のあと必ず黒画面になります（本体の仕様）。
