...
```
SHA256
PocketRoles-Setup-0.5.0.zip  <SHA256 PocketRoles-Setup-0.5.0.zip>
PocketRoles-0.5.0.zip        <SHA256 PocketRoles-0.5.0.zip>
```
...
- 実機（PC ホスト＋エミュレータ 2 台）で確認済み: マッドホーク・イビルホークの視界、崇拝者（崇拝・ホストの自爆・シェリフに撃たれる・Ⓦ）、ジャッカルフレンズの勝敗カウント、侍（クライアントのタイマー再開・desync ホストを斬る・ホスト自身のクールダウン 45 秒）、シリアルキラーのタイマー。未確認: 侍が 3 人以上をまとめて斬るケース（4 人必要）、マッドホークの速度設定。
