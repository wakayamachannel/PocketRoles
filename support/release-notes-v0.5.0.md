...
- <§12 の実機テストの結果をここに: ゲート G1〜G9 の判定（設定タブの「マッド系を撃てる」の表示幅、マッドホーク / イビルホークの視界が参加者の画面で広がること、崇拝者のホスト自爆、Ⓦ の表示、侍のクールダウン復帰、ホストのシェリフを斬った時、シリアルキラーのボタン再設定、連続キルでのキック無し）と、ゲートで外した役職・戻した設定があればその一覧>
