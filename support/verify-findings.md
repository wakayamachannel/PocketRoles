...
| 54 | **解決（v0.4.3, 2026-09-09 実機）** | 登録オフ（便利ホスト）の部屋で参加者が入室した瞬間にホストが Hacking 切断。**真因**: 入室時の歓迎チャット（公開 SendChat）に、バニラのチャット欄が受け付けない文字（`[` `]` `！` `（` `）`）が含まれていた。公式サーバーは「プレイヤーが打てない文字を含む公開チャットをホストが流す＝不正」と判定して 70ms 後にホストを切断していた。パケット単位のフック（[Diagnostics] WireLog）で「歓迎チャット → KickPlayer(reason=Hacking)」を直接観測し、文字を除くと切断が消えることを確認。長さ（190B メッセージも通る）や RPC バイト列は無関係。**対策**: `Rpc.SanitizeForVanillaChat` が互換モードの全チャットをバニラで打てる文字に整える（全角→半角、`[]`→`【】`、`\| _ ~ "` を代替、`TextBoxTMP.IsCharAllowed` で落とす）。互換モードの `@name` は打てない `@` を避けて `name:` に。バニラのチャット記号ホワイトリスト（2026.8.18）= `?!,.':;()/\%^&-=¿？#`。**確認**: PC ホスト＋MuMu 2 台で、プレーン Skeld・逆スケルド両方の便利ホスト部屋に入室 → 切断なし、歓迎文・/cmd 返信も配信。登録オフでも逆スケルド可。役職は従来どおり登録オンの部屋のみ |
...
### #55 (2026-09-09 20:34, live game, RESOLVED 21:39 in v0.4.5) 遅延キル（ヴァンパイアの噛みつき）が会議開始と同じ瞬間に自己キルとして実行され、被害者の画面が暗転したまま戻らない
- 再現 (21:23, v0.4.4, PC ホスト + MuMu 2 台, `/test on`, `/assign 0 lighter; 1 sheriff; 2 vampire`): エミュ 2 が噛む → 10 秒後にホストが緊急ボタン → `bite executes` の MurderPlayer(self) と StartMeeting が 9 ms 差 → 被害者(エミュ 1)の会議画面は緊急会議カットシーンのあと壊れたまま（自分の投票枠なし・パネルが斜めに固まる）。噛みつきだけ（会議なし）なら自己キル表示は正常（21:34）→ 衝突が原因と確定。
- 修正: `Kills.LastMurderAt` + `Meetings_ReportDeadBodyPatch` が `KillMeetingGap` 秒待つ。1.6 s では +9 s でもパネルが半描画（+60 s で回復）、**3.0 s で +6 s に通常の「死亡」会議イントロ → +30 s 完全表示** (21:39)。ホストの緊急ボタン 3 回連続もそのまま動作。テスト中の副産物: 3 人部屋でインポスター系 2 人だと AntiBlackout の view 復元で全クライアントが会議後に暗転する（既知の制限、#54 前後に記載の 2 人問題の親戚）。
- 補足: MuMu の `input swipe` ではジョイスティックが動かず、`input motionevent DOWN/MOVE/UP`（長押し）なら動く。
- 観察: 7 人部屋。ヴァンパイア(ホスト)が BrainPower(シェリフ) を噛む → 13 秒後に `Kills: bite ... executes` → mod は被害者自身を killer にした `MurderPlayer(net=34→34, flags=9)` を送信（20:34:28.441）。その 21 ms 後にホストの `StartMeeting(0xFF)`（緊急ボタン、直前に SnapTo）。被害者は「自分で自分を撃った＝シェリフの自爆」に見え、キル演出中に会議が始まって画面が暗転したまま（本人報告「シェリフで自爆したら画面が暗転しつづける」）。その後 EndGame reason=5 (ImpostorDisconnect)。
- 仮説: 2026.8.18 クライアントは KillAnimation コルーチン中に MeetingHud が生成されると KillOverlay/カメラのフェードが戻らない。自己キル自体は #1 試合のラバーズ道連れで正常動作しており、会議との衝突が原因。
- 対策案 (v0.4.5): (1) ホストが StartMeeting を送る直前 2 秒以内に MurderPlayer を送っていたら、その差分だけ StartMeeting を遅らせる（2026 は会議がホスト承認制なので可能）。(2) 遅延キル（噛みつき・ラバーズ道連れ・侍など）は「会議開始待ち／会議中」なら会議明けまで保留。(3) 噛みつきの killer を被害者自身ではなくヴァンパイア本人にする案は、シェリフ誤射との見分け（本人には「自爆」に見える）の面でも検討。

## v0.5.5 公開前の実機テストに足す項目（2026-09-23、設計レビュー §5.5 / §8 の修正）

PC（ホスト）＋エミュレーターで確かめます。音は消し、始める前にオーナーへ知らせます。

**これは不具合の一覧ではなく、公開前に試す手順です**。上の 1〜55 と混ざらないように、番号は `T1`〜`T6` にしています。

| # | 内容 | 確認方法（PC ＋ エミュレーター） |
|---|---|---|
| T1 | **試合中の `/cmd n` は最後の役職説明だけ**（`Commands.MyRoleText` + `RoleCmdCore.Decide`）。生きている人が会議の外で打っても、その時の状態（生きているジャッカル、耐えられるキルの残り、崇拝の残り）が出ないこと | 登録ありの部屋、`/test on` と `/assign` でエミュレーター 1 をジャッカルフレンズ、エミュレーター 2（または PC）をジャッカルに。開始 → 役職説明が届くのを待つ（開始の約 8 秒後）→ ジャッカルを死なせる → **会議を開かずに** エミュレーター 1 から `/cmd n`。返事が開始時と同じ「ジャッカル: 〇〇」のままで、「生きているジャッカルがいません。」が出ないこと。ログに `Commands: /cmd n of #… answered with the last role text sent (alive, outside a meeting)` が出ること。※エミュレーターはバニラなので試合中はチャット欄が開きません。試合中の送信は PC 側（ホスト以外の 2 台目の PC のバニラ）か、ホストの `/ac test chat` と同じ経路で確かめるか、**手順だけ確認して「エミュレーターでは再現できない」と記録**します |
| T2 | **イントロ中と追放の画面の最中も「試合中」**（オーナーが名指しした場面。`Commands.InMeeting` は `MeetingHud` に加えて `ExileController` と `IntroCutscene` も見ます）。この 2 つの画面で `/cmd n` を打っても、その時の状態が出ないこと | T1 と同じ部屋で 2 回。(1) 開始直後のイントロ中に `/cmd n` → 役職説明（状態を読む行なし、または開始時の文）が返ること。(2) 会議で誰かを追放し、**追放の演出が流れている間**に `/cmd n` → 直前の会議で届いた文がそのまま返ること（追放されたのがジャッカルでも「生きているジャッカルがいません。」にならない）。どちらもログに `answered with the last role text sent (alive, outside a meeting)` か `role description only` が出ること。※前の `/cmd n` から 10 秒あけてから打つこと（T4） |
| T3 | **返す中身は今まで通り**（会議・死亡・ロビー・終了画面）。T1 の続きで、会議を開いてエミュレーター 1 が `/cmd n` → 今度は「生きているジャッカルがいません。」が出ること。エミュレーター 1 を死なせてから `/cmd n` → その時の状態が出ること。ロビーと試合の終わりの画面で `/cmd n` → 「試合が始まってから使えます。」（終了画面も同じ文です） | 同じ試合の続き。**前の `/cmd n` から 10 秒以上あけてから打つこと**（10 秒以内は場面を問わず黙って捨てられます。T4） |
| T4 | **`/cmd n` の連投制限**（同じ人に 10 秒に 1 回。ホストは制限なし）。既存の 2 秒制限だけでも通ってしまわないよう、間隔をあけて試します | 会議中に手で: **3 秒おきに 3 回** `/cmd n` → 返事は**最初の 1 回だけ**（2 秒制限しか効いていなければ 2 回目・3 回目も返ってしまいます）。最後から **11 秒後**にもう 1 回 → また返ること。ほかのコマンド（`/cmd h`、`/cmd s`）は今まで通り 2 秒の間隔で返ること。ホスト自身の `/cmd n` は連投しても毎回返ること |
| T5 | **地域の自動選択は「部屋を作る」時だけ**（`AutoRegion_OpenCreateGamePatch`）。`/opt lobby.autoregion on` にしてゲームを再起動 → **オンライン**を開くだけでは何も起きない（ログに `AutoRegion:` が出ない、地域が変わらない）→ **部屋コードで参加**しても変わらない → 「部屋を作る」を開いた時だけ計測が走り、最速の地域に切り替わること（ログ `AutoRegion: … → best=…`、`SetRegion(…)`）。切り替わったら「部屋を作る」画面の地域表示もその場で変わること（`SetCurrentServer`。変わらなくてもエラーが出ていなければ許容）。**画面を開いてすぐ「作成」を押した時**は、その部屋は元の地域のままで、ロビーのチャットに「次に『部屋を作る』の画面を開いた時に切り替えます」と出ること。そのあともう一度「部屋を作る」を開くと、計測をやり直さずにその場で切り替わること（ログ `pending switch from the cached probe`）。**いちばん大事な場合**: 「部屋を作る」を開いてすぐ「戻る」→ 部屋コードで参加 → 参加している最中にログの `AutoRegion: … → best=…` が出ても、`SetRegion(` は出ず地域が変わらないこと（計測が終わった時に画面が閉じているため） | PC 単独。**計測結果は 10 分キャッシュされます**: やり直す時はゲームを再起動するか 10 分あけてください（10 分以内の 2 回目は計測が走らないのが正常です）。確かめたら `/opt lobby.autoregion off` に戻す（既定はオフ） |
| T6 | T5 の続き: 部屋を作った後のロビーのチャット（ホストのみ）に「地域の自動選択:」の表が 1 回出ること、`/region` の表示が「現在の地域」「応答時間（中央値）」「自動で最速の地域を選ぶ: オン」になること。オフの時は「応答時間はまだ計測していません（「部屋を作る」の画面を開いた時に計測します）。」 | ロビーで `/region`。**この表は計測が走った時だけ積まれます**: 10 分キャッシュ中に作り直した 2 つ目の部屋では出ないのが正常です（不具合ではありません）。出させたい時はゲームを再起動してから部屋を作ってください |
