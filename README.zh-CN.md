...
**这是一个只需建房的人安装的 Among Us（2026.8.18 / Steam）职业模组。** 朋友们用 PC、手机或 Switch 上平时的 Among Us，输入房间代码就能加入。警长、豺狼、小丑等 13 种职业会通过名字标签和聊天悄悄地只告诉本人。设置在房间的电脑里完成，提示支持 日本語 / 中文 / English 三种语言，外语聊天自动翻译，房间超时和拖沓的会议也能由房主一键处理。免费、非营利，源代码在 GitHub 公开（**v0.4.0**，旧名 HostRoles）。
...
- 新增 13 个职业：警长、市长、告密者、点灯人、增速者、诱饵、内鬼狂粉、吸血鬼、黑手党、小丑、投机者、恐怖分子、豺狼
...
- 警长和豺狼在自己的客户端上以“内鬼”身份运行（为了拥有击杀键），而在其他所有人眼中是“船员”。
...
6. 标题画面右侧出现 PocketRoles 面板、左上角显示 `PocketRoles v0.4.0` 即安装完成。
...
- 如果游戏 UI 变化导致面板无法创建，会改为显示 v0.2 的右下角署名行（`PocketRoles v0.4.0  © 2026 もみじちゃ` 和网址），并上移以免与原版的版本显示重叠。
- 不发送任何数据（仅房主屏幕）。
...
| 职业 | 阵营 | 说明 | 主要设置（默认值） |
|---|---|---|---|
| 警长 (Sheriff / シェリフ) | 船员 | 可用击杀键射杀内鬼（含吸血鬼、黑手党）和豺狼。射杀不能杀的对象（船员、中立阵营）时自己会死。不能跳管、不能破坏。**任务是假的**（不计入进度）。 | 击杀冷却 30 秒，可击杀内鬼狂粉：on |
...
聊天中 `/cmd r` 显示列表，`/cmd r <职业名>` 显示说明和当前设置。职业名可以用英文名、日文名、中文名或缩写（`sh`、`my`、`sn`、`lt`、`sb`、`bt`、`mad`、`vamp`、`mf`、`js`、`opp`、`tr`、`jk` 等）中的任意一种。日文名、中文名只输入开头几个字也可以（`シェリ`、`警`）。
...
4. 指定想测试的职业。可以用名字，也可以用玩家编号（`/assign show` 或按颜色顺序的编号）。名字支持部分匹配（能唯一确定一人时）。
   ...
   职业可以不受正常抽选名额限制地指定。把内鬼名额的职业（吸血鬼、黑手党）指定给船员时，该玩家会以内鬼身份开始。指定在一局游戏中消耗掉（下一局需要重新指定）。
...
1. **标题画面面板**：PC 主菜单右侧出现 PocketRoles 面板（图标、`v0.4.0 / Among Us 2026.8.18`、GitHub 行），点击 GitHub 行会打开浏览器。打开在线菜单时隐藏，返回后重新显示。
...
| 职业 | 开场动画的显示 |
|---|---|
| 警长、豺狼 | **“内鬼”**（红色画面，队友只有自己）。因为本人的客户端以内鬼身份运行。真正的职业会在开场动画结束后通过名字标签和聊天告知 |
| 吸血鬼、黑手党 | 普通的内鬼开场动画（也会显示其他内鬼） |
| 内鬼狂粉、小丑、投机者、恐怖分子、市长、告密者、点灯人、增速者、诱饵 | 普通的“船员”开场动画 |
| 普通内鬼 | 普通的开场动画。但警长、豺狼不会显示为队友，看起来是船员 |
...
| 职业 | 任务的显示 |
|---|---|
| 警长、豺狼 | 客户端上是内鬼，所以是 **假任务**（与内鬼相同的显示）。无法使用任务终端。不计入船员任务进度 |
| 小丑、投机者、内鬼狂粉 | 正常显示任务，也可以操作，但 **不计入** 船员任务进度（本人看不出区别。说明文中写有“任务是假的”） |
...
- 警长（不能跳管、不能破坏）、豺狼（不能破坏，跳管按设置）、内鬼狂粉（与原版船员相同）：客户端上是内鬼的警长、豺狼 **会显示** 跳管和破坏按钮、地图，但使用时会被房主拒绝。进入管道 0.5 秒后会被弹出，破坏和关门会被忽略。每 10 秒最多 1 次聊天提示“你的职业不能跳管。”“你的职业不能破坏。”
- **开门** 操作任何人都可以。
...
- 房主的画面上会以阵营颜色显示真正的结果（“小丑获胜”“豺狼获胜”等）。
...
- 警长和豺狼的开场动画显示为“内鬼”（队友只有自己）（[第 26 章](#26-原版玩家看到的是什么)）。在警长、豺狼眼中 **其他所有人都是船员**。有多个豺狼时也无法互相识别。真正的职业会在开场动画之后立刻通过名字标签和聊天送达（提前告诉玩家“开场动画不一定准”会更友好）。
...
- 主机、Switch、手机上只能使用快捷聊天的玩家 **可以阅读** 职业通知和命令回复，**但不能输入命令**（也无法使用 `/lang`，这类参与者较多时请把房间默认语言调整到合适的语言）。
...
- 职业只从“普通船员”“普通内鬼”名额中分配。成为原版特殊职业（科学家、工程师、变形者等）的玩家不会获得特殊职业。吸血鬼和黑手党占用内鬼名额，所以内鬼设为 1 人、吸血鬼设为 1 时，那 1 人就是吸血鬼（`/assign` 不受此限制）。
- 警长、豺狼、点灯人、增速者的效果是作为该玩家专属的“游戏设置”发送的，所以在房间设置变更等导致原版设置重新发送后，可能暂时恢复为普通值（游戏开始时、开场动画结束时、会议后会重新发送）。
...
2. 在弹出的对话框中点 **“打开邮件 (问题)”**（或“(建议)”），会打开邮件客户端，收件人、主题（`[PocketRoles] 问题报告 v0.4.0`）和正文模板（发生了什么 / 何时、房间代码、人数 / 玩家看到了什么）已填好。**请从桌面把 zip 添加为附件**（没有邮件客户端时，点“打开 zip 所在位置”找到文件，用浏览器里的 Gmail 等网页邮箱发送即可）。
