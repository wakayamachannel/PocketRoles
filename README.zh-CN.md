...
- 想玩职业请创建 **注册开启（默认）** 的房间。招人的方法见 [第 25 章](#25-便利房关闭注册与引导房)（引导房、`/move`）。
...
- 希望被服务器断开时自动重建房间，请用 `/rehost on`（[第 15 章](#15-自动重建房间与自动公开)）。重建后代码会变，记得改引导房的名字。
...
- 原房间是公开的话，新房间也会恢复为公开。
- 新房间创建后，房主的聊天中会显示“断线后已自动重新创建房间。新房间代码：XXXXXX”。**房间代码会改变**，请重新告诉参与者（用 `/announce` 复制后改引导房的名字，[第 25 章](#25-便利房关闭注册与引导房)）。应对房间超时请使用废村（[第 16 章](#16-房间剩余时间自动开始与废村)），这样代码不会变。
...
职业房不会出现在公开列表里，所以用 **副手机（或另一台 PC）开一个原版公开房，把职业房的代码写在名字和聊天里** 来招人。玩家在引导房看到代码后，再加入职业房。
...
- 设为 `[Guide] AutoRecreateRegistered = true`（设置标签页“/move 后重建为注册房”，`/opt guide.autoreg on`）后，`/move` 的 30 秒后 **本房间会重建为注册房间**（“30 秒后本房间将重建为有职业的房间，请用新代码重新加入”→ 5 秒前再次提醒 → 重建）。新代码会显示在房主的聊天里，用 `/announce` 抄到引导房。默认关闭（只发引导）。
...
- 自动重建、`/move`、延迟过高的重建产生的房间 **房间代码会改变**（废村则不会改变）。请改引导房的名字。
