...
- **13 役職を 3 言語でこっそり通知** — シェリフ・メイヤー・スニッチ・ジャッカル・ジェスター…。役職の説明は日本語 / 中文 / English で本人にだけ届き、外国語のチャットは自動翻訳（併用モード）
- **ホストが楽になる道具つき** — ロビー残り時間の常時表示と自動延長、自動開始、廃村（F7）、会議の強制終了（F8）、開始キャンセル（F9）、部屋コードの大表示、友達用インストーラー

| ![メインメニューのパネル](docs/img/menu-panel.png) | ![ロビー残り時間と部屋コード](docs/img/lobby-timer.png) | ![設定タブ](docs/img/settings-tabs.png) |
|---|---|---|
| タイトル画面の PocketRoles パネル | ロビー残り時間と「役職部屋 ABCDEF」の大表示 | ロビーのパソコンの設定タブ（役職 / ロビー / 会話 / 見た目 / ホスト） |
...
1. **[GitHub Releases](https://github.com/wakayamachannel/PocketRoles/releases)** から 2 つの zip をダウンロードします: `PocketRoles-Setup-0.4.0.zip`（ランチャー）と `PocketRoles-0.4.0.zip`（MOD 本体）。
...
役職ありの部屋は「登録済み MOD 部屋」なので **公開一覧には出ません**（Innersloth のポリシー、[第 3 章](#3-innersloth-の-mod-ポリシーと公開部屋についての注意必読)）。そこで、**サブのスマホ 1 台** で「案内部屋」を作って人を呼びます。

1. **PC で役職部屋を作ります**（いつも通り。登録はオンのまま）。ロビーの左上に「役職部屋 QWERTY」のように **部屋コードが大きく** 出ます。チャットに `/announce` と打つとコードがクリップボードにコピーされ、下の手順も表示されます。
...
| 部屋が公開一覧に出ない | 登録済みの MOD 部屋は一覧に出ません。上の「案内部屋」の方法で呼ぶか、部屋コードを直接伝えてください（[第 25 章](#25-便利ホスト登録オフと案内部屋)） |
...
| 規約違反にならない？ BAN されない？ | Innersloth のポリシーどおり、部屋作成時に MOD 登録（+25）します。登録したままなら利用だけで BAN はされません（[第 3 章](#3-innersloth-の-mod-ポリシーと公開部屋についての注意必読)） |
...
- 追加役職 13 種: シェリフ、メイヤー、スニッチ、ライター、スピードブースター、ベイト、マッドメイト、ヴァンパイア、マフィア、ジェスター、オポチュニスト、テロリスト、ジャッカル
...
- **案内部屋の支援**: 部屋コードの大表示（`/code`）、コードのコピーと手順表示（`/announce`）、便利ホスト部屋から役職部屋への移動案内（`/move`）
...
- シェリフとジャッカルは、本人のクライアント上では「インポスター」として動作し（キルボタンのため）、他の全員からは「クルー」に見えます。
...
- 役職を配れるのは **登録済み（+25）の部屋だけ** です。登録オフの部屋（便利ホスト）では個別メッセージを送るとホストがサーバーに切断されるため、役職・個別通知は無効になり、バニラの進行にホスト支援ツールだけが付きます（[第 25 章](#25-便利ホスト登録オフと案内部屋)）。
- ホストの画面左上（ping 表示）に `PocketRoles v0.4.0 (host) · アジア` のように接続中の地域名と、オンラインのロビーでは `ロビー残り mm:ss` と大きな部屋コード（`役職部屋 ABCDEF`、案内部屋に書き写す用）が表示され、ゲーム内の MOD スタンプも表示されます。タイトル画面では右側の大きな窓に **PocketRoles のパネル**（アイコン、`v0.4.0 / Among Us 2026.8.18`、作者名、クリックで開く GitHub の行、「ホストだけ導入で役職が遊べる MOD」「参加者はバニラのままで OK」）が出ます（[第 9 章](#9-歯車メニューのpocketroles-設定パネル)）。
...
- **案内部屋の支援**（[第 25 章](#25-便利ホスト登録オフと案内部屋)）: ロビーの左上に部屋コードを大きく表示（`役職部屋 ABCDEF`、`/code` で切替、設定タブ「部屋コードを大きく表示」）。`/announce`（`/guide`）でコードをクリップボードにコピーして、サブスマホで案内部屋を作る 4 つの手順を表示。`/move [コード]` で便利ホスト部屋から役職部屋への移動案内を 3 言語で全員に送信（`/opt guide.autoreg on` なら 30 秒後に登録部屋として作り直し）。歯車メニューにも案内部屋のヒント。
...
- **役職の配り方を修正**: 2 人以上で開始した時にイントロが出ず真っ暗になる問題を修正（本体の役職送信をそのまま通し、直後に全員分の見え方をまとめて上書き）。シェリフ・ジャッカルのイントロが「インポスター」になるのは従来どおりです。
- **テストモード**: `/test on` で開始ボタンが即「開始」になり、本体の「4 人でプレイ可能ですが…」ポップアップも自動で OK。
- **言語ファイル**: アップデートで増えたキーを起動時に `lang\*.json` へ追記します（自分で編集した文章はそのまま）。
- **ログ**: 通常は静かに。`/diag on` で開始処理の詳細トレース、`/diag` で状態のスナップショット。
...
| 言語の自動判定 / 3 言語の案内 | `/lang` を使っていない参加者が中国語・英語で書くと表示言語を自動で切り替えて案内。挨拶に「English: /cmd lang en ｜ 中文: … ｜ 日本語: …」の 1 行。`WelcomeAllLanguages` で挨拶を 3 言語で送ることも可 | [13](#13-言語日本語--中文--english), [14](#14-参加時の挨拶文とルール行) |
...
| バニラ設定の範囲拡張 | キルクールダウン 0〜120 秒（0.5 秒刻み）、投票 0〜600 秒、会議 0〜600 秒、緊急会議 CD 0〜120 秒、タスク数 0〜30 などを設定画面の矢印と `/vset` で。移動速度・視界も `/vset` で拡張。バニラの参加者にも同じ値が届きます | [8](#8-設定タブロビーの設定画面), [11](#11-コマンド一覧) |
...
PocketRoles は役職を追加し、他のプレイヤーの体験を変える MOD なので、公式サーバーで使うには **部屋作成時の登録が必須** です。
ホスト専用 MOD の登録方法は、プロトコルバージョンに 25 を加える「host authority mode」（+25 フラグ）で、Innersloth の公式資料 <https://github.com/Innersloth-LLC/AmongUsModdingInformation> に説明があります。PocketRoles は **既定でこれを行います**（設定 `RegisterAsModdedLobby = true`。部屋を作る時だけ付き、他人の部屋に参加する時は付きません）。
...
登録（+25）した PocketRoles の部屋は、バニラの公開部屋一覧に **表示されません**（スマホ版で確認済み）。「公開」にしても同じです。参加者には **ルームコード** を伝えるか、[案内部屋](#25-便利ホスト登録オフと案内部屋)（サブスマホのバニラ公開部屋に「役職→コード」と書いておく方法）で呼んでください。
...
技術的な補足（2026.8.18 のクライアントを調べた結果）: バニラの「部屋を探す」画面のフィルタには MOD 用の項目（ModFilter）があり、これは全員が同じ MOD を入れる GUID 登録型の MOD が使うものです。バニラのクライアントはこのフィルタを付けないので、登録（+25）したホスト専用 MOD の部屋はサーバー側で一覧から除外されます。登録したまま一覧に載せる方法はクライアント側にはありません。

### 3.3 登録をオフにすると「便利ホスト」（役職なし）になります

`RegisterAsModdedLobby = false`（`/opt register off`、設定タブの「MOD登録(+25)」、歯車パネルの「登録(+25)」）で作った部屋は公開一覧に載りますが、**役職は配られません**。
...
- 役職ありで遊ぶには **登録オン（既定）** の部屋を作ってください。人を集める方法は [第 25 章](#25-便利ホスト登録オフと案内部屋)（案内部屋、`/move`）。
...
AUR（Among Us Revamped）は役職や個別設定の送信（desync）を含まないホスト用の **ユーティリティ MOD** で、+25 フラグも付けていません。PocketRoles は「他のプレイヤーの体験を変える MOD」なので、ポリシー上の扱いが異なります。v0.4 のホスト支援ツール（廃村、キャンセルボタン、ゲームマスターなど）は AUR / EHR / SuperNewRoles と同じバニラの仕組みで動きます。
...
4. ランチャーの **「インストール」** を押します。次を自動で行います（数分。進行状況は下の欄に出ます）。
   1. Steam 版の Among Us を探して（見つからない時はフォルダ選択）、`デスクトップ\Among Us PocketRoles` にコピー（1 GB 前後）
...
6. タイトル画面の右側に PocketRoles のパネル、左上に `PocketRoles v0.4.0` が出れば導入完了です。
...
4. GitHub Releases の `PocketRoles-<ver>.zip` をコピー先に展開します（`BepInEx\plugins\PocketRoles.dll` と `BepInEx\PocketRoles\lang\*.json`、README、LICENSE が入っています）。**旧版の `HostRoles.dll` が残っている場合は削除してください**（同じパッチが二重に当たります）。
5. コピー先の `Among Us.exe` を起動します（Steam ライブラリからではなく、コピー先の exe を直接。Steam は起動しておく）。画面左上に `PocketRoles v0.4.0`、タイトル画面の右側の窓に PocketRoles のパネルが出れば MOD が読み込まれています。`BepInEx\LogOutput.log` に `PocketRoles v0.4.0 loaded` と出ていることでも確認できます。
...
4. 人を集めます。ロビーの左上に **部屋コードが大きく**（`役職部屋 ABCDEF`）出ているので、友達に伝えるか、サブスマホの **案内部屋** に書き写します（`/announce` でコピー。[第 25 章](#25-便利ホスト登録オフと案内部屋)）。参加した人には数秒後に、MOD 部屋であることと有効な役職・ルール・言語の切り替え方の案内が **その人にだけ** チャットで届きます（[第 14 章](#14-参加時の挨拶文とルール行)）。
...
- サーバーから切断された時に自動で部屋を作り直したい場合は `/rehost on`（[第 15 章](#15-自動再ホストと自動公開)）。作り直すとコードが変わるので、案内部屋の名前も直してください。
...
- **役職**（役職色の見出し）: 人数（0〜15）、確率（0〜100%、5 刻み）、その役職の固有設定（シェリフ: キルクールダウン・マッドメイトを撃てる、ジャッカル: キルクールダウン・ベント使用、ヴァンパイア: 噛みつき遅延、メイヤー: 票数、スニッチ: 残りタスクで警告、ライター: 視界倍率、スピードブースター: 速度倍率、マッドメイト: インポスターに公開）
...
- **ホスト（ホスト支援）**: 上に **操作ボタンの列**（今すぐ開始 / キャンセル / 廃村 / 会議終了 / テストモード / 設定を表示。ロビーや試合の状態に応じて押せるものだけ有効）。見出し「全般」= MOD登録(+25)、言語（ja / zh / en）、参加時の挨拶、会議で役職説明、不正RPCでキック、バージョン不一致を無視、クレジット表示。見出し「ホスト支援」= ゲームマスター、ホットキー有効、廃村キー(2回押し)、会議終了キー(2回押し)、開始キャンセルキー（それぞれ F1〜F12 から選択）、**部屋コードを大きく表示、/move 後に登録部屋へ作り直す**（案内部屋、[第 25 章](#25-便利ホスト登録オフと案内部屋)）、**アドミンにも設定変更を許可、モデレーターのキック/BAN、VIP の名前に★**（権限）、**バニラ設定の範囲拡張、キルCD最小(秒)、キルCD最大(秒)、キルCDの刻み(秒)、投票時間最小(秒)、投票時間最大(秒)、議論時間最大(秒)、緊急会議CD最大(秒)、タスク数最大**（下の「バニラ設定の範囲拡張」）
...
|---|---|---|---|
| キルクールダウン | 10〜60 秒、2.5 刻み | **0〜120 秒、0.5 刻み** | `KillCooldownMin`（0〜60）/ `KillCooldownMax`（10〜600）/ `KillCooldownStep`（0.5〜10） |
...
| 登録(+25) | MOD 登録（オフはポリシー違反。次に作る部屋から） | `[General] RegisterAsModdedLobby` |
...
| `v0.4.0 / Among Us 2026.8.18` | MOD のバージョンと対応するゲームバージョン |
...
| 「ホストだけ導入で役職が遊べる MOD」「参加者はバニラのままで OK」 | 部屋の既定言語で表示 |
...
- ゲームの UI が変わってパネルが作れなかった時は、v0.2 の右下のクレジット行（`PocketRoles v0.4.0  © 2026 もみじちゃ` と URL）が代わりに出ます（バニラのバージョン表示と重ならないように上へ寄せます）。
- 何も送信しません（ホストの画面だけ）。
...
| 役職 | 陣営 | 説明 | 主な設定（既定値） |
|---|---|---|---|
| シェリフ (Sheriff / 警长) | クルー | キルボタンでインポスター（ヴァンパイア・マフィア含む）とジャッカルを撃てます。撃てない相手（クルー、第三陣営）を撃つと自分が死にます。ベント・サボタージュは不可。**タスクは偽物**（進行に数えません）。 | キルCD 30 秒、マッドメイトを撃てる: on |
...
| マッドメイト (Madmate / 内鬼狂粉) | インポスター | インポスター陣営のクルー。インポスターの名前が赤く見えますがキルはできません。インポスターが勝つとあなたも勝ちです。タスクは進行に数えられず、インポスター勝利の人数判定でもクルーに数えられません。 | インポスターに公開: off |
...
チャットの `/cmd r` で一覧、`/cmd r <役職名>` で説明と現在の設定が見られます。役職名は英語名、日本語名、中国語名、短縮形（`sh`、`my`、`sn`、`lt`、`sb`、`bt`、`mad`、`vamp`、`mf`、`js`、`opp`、`tr`、`jk` など）のどれでも指定できます。日本語名・中国語名は先頭の数文字だけでも通ります（`シェリ`、`警`）。
...
| インポスター側のキラー（インポスター、ヴァンパイア、マフィア）とジャッカルが全滅 | クルー勝利 |
| ジャッカルがいない状態で、インポスター側キラーの数 ≥ それ以外の生存者数（マッドメイトを除く） | インポスター勝利（マッドメイトも勝ち） |
| インポスター側キラーがいない状態で、ジャッカルの数 ≥ それ以外の生存者数（マッドメイトを除く） | ジャッカル勝利 |
| ジェスターが追放された | ジェスター単独勝利 |
| タスクを終えたテロリストがキルまたは追放された | テロリスト単独勝利 |
| 上記のどれかで終了した時に生きているオポチュニスト | 勝者に追加 |

インポスターとジャッカルの両方が生きている間は、人数が少なくても試合は続きます。テストモード中は緊急サボタージュのタイマー切れと `/end` 以外では終了しません。ホストが F7 ×2 / `/haison` で終了した試合は勝敗なし（結果一覧も出ません）。
...
- 参加者のコマンドは 1 人につき 2 秒に 1 回まで。返答は 1 コマンドにつき最大 3 通（挨拶文のプレビューは 4〜5 通）で、**その人の言語** で届きます。
...
| `welcome settings on|off` | 挨拶に現在の設定を付けるかどうか |
...
| `code` / `code on|off` | ロビー左上の **部屋コードの大表示** を切り替え（`[Guide] ShowCodeOverlay`）。現在のコードもチャットに表示 |
| `announce`, `guide`, `案内` | 部屋コードを **クリップボードにコピー** し、案内部屋（サブスマホ）の作り方 4 手順を表示（[第 25 章](#25-便利ホスト登録オフと案内部屋)）。登録オフの部屋では `/move <コード>` で設定した役職部屋のコードをコピー |
...
| `diag` / `diag on|off` | 開始ボタン・テストモード・廃村・試合の状態と画面のスナップショットをチャットとログに出す（真っ暗な時の報告用）。`on` で開始処理の詳細トレースをログに出し、`off` で止める |
...
| `chat.welcomeall` | on / off | `[Chat] WelcomeAllLanguages`（挨拶を ja / zh / en の 3 言語で送る） |
...
| `guide.overlay` | on / off | `[Guide] ShowCodeOverlay`（部屋コードの大表示。`/code` と同じ） |
...
RegisterAsModdedLobby = true    # 部屋作成時に +25 フラグで登録（Innersloth のポリシーで必須。off はポリシー違反）
...
WelcomeIncludeSettings = true   # 挨拶に現在の役職設定を付ける
...
WelcomeAllLanguages = false     # 挨拶を日本語・中国語・英語の 3 言語で送る（false = 既定言語 + 3 言語の /lang 案内 1 行）
...
KillCooldownStep = 0.5          # 矢印 1 回の刻み（秒、0.5〜10。バニラ 2.5）
...
[Guide]                         # 案内部屋の支援（v0.4e）
ShowCodeOverlay = true          # ロビー中、ホストの画面左上に部屋コードを大きく表示（/code）
RoleRoomCode =                  # 便利ホスト部屋から /move で案内する役職部屋のコード（空 = 案内部屋のホスト名を見てもらう）
AutoRecreateRegistered = false  # /move の 30 秒後にこの部屋を登録(+25)の部屋として作り直す
...
CanKillMadmate = true           # マッドメイトを撃っても死なない
...
[Madmate]
KnownToImpostors = false        # インポスターがマッドメイトを見分けられる
```
...
参加者は `/cmd lang en`（登録済みの部屋なら非公開）または `/lang en` で **自分宛てのメッセージだけ** を別の言語にできます。挨拶文には `/lang en|zh|ja で言語変更` の案内と、3 言語で書いた 1 行 `English: /cmd lang en ｜ 中文: /cmd lang zh ｜ 日本語: /cmd lang ja`（v0.4b）が入っているので、日本語が読めない人もすぐに切り替えられます。
...
- 日本語・中国語のヘルプには短い英語の行（`EN: …`）が付きます（挨拶は設定行を付けない時だけ）。
...
- **挨拶を 3 言語で送る**（`[Chat] WelcomeAllLanguages = true`、設定タブ「挨拶を3言語で送る」、`/opt chat.welcomeall on`）: 挨拶文全体を、その人の言語 → 残りの 2 言語の順に 3 回送ります（最大 3 倍の通数になるので、順番に届きます）。この時は 3 言語の案内行と `EN:` の行は付きません。
...
```json
{
  ...
  "welcome.1": "この部屋はホスト専用MOD「PocketRoles」を使用しています。役職付きの試合になります。",
...
参加者がロビーに入った 3 秒後、その人にだけ挨拶文が届きます（`WelcomeMessage = true` の時。複数人が同時に入っても順番に送ります）。1 通 100 文字以内で最大 4 通（設定行を付ける時は 5 通。翻訳の案内行と 3 言語の案内行があればそれぞれ +1 通、VIP の行があれば +1 通）です。

**標準の挨拶文**（既定言語が日本語、既定の設定の場合）:

```
この部屋はホスト専用MOD「PocketRoles」を使用しています。役職付きの試合になります。
/cmd h でヘルプ（ホストにだけ届きます）。/lang en|zh|ja で言語変更。
有効な役職: シェリフ、マッドメイト、ジェスター
...
シェリフ x1 キルCD30秒 マッド可
マッドメイト x1
ジェスター x1
言語=ja 登録(+25)=on 挨拶=on
```

- 4 行目が **ルール行** です（v0.4）。既定は「ルールなし」の文で、`/rules <文章>` で自分のルールに置き換えられます。
- 5 行目は **翻訳の案内**（チャット翻訳がオンの時だけ。[第 13 章](#13-言語日本語--中文--english)）、6 行目は **3 言語の言語切り替え案内**（参加者がコマンドを使える時だけ。登録オフの部屋では `/lang en` の形）です（v0.4b）。
- 設定行（後半）は `WelcomeIncludeSettings`（`/welcome settings on|off`、設定タブの「挨拶に設定を含める」）で外せます。自動再ホスト / 自動公開、自動開始 / ゲームマスターがオンの時はその行も付きます。設定行を付けない場合は代わりに短い英語の案内行（`EN: …`）が付きます。
- `VIP.txt` に載っている人には最後に「★ VIP ○○ さん、ようこそ！いつもありがとうございます。」が付きます（[第 11 章](#11-コマンド一覧)）。
- `WelcomeAllLanguages = true` の時は、この挨拶がその人の言語 → 残り 2 言語の順に 3 回届きます（案内行と `EN:` 行は無し）。
...
例: `/rules 初心者歓迎！\n暴言・晒しは禁止です。` → 挨拶文の 4 行目に入ります（`/welcome show` でプレビュー）。標準のルール文は言語ごとに翻訳されますが、自分で書いた文はそのままです。

### 自分の文章にする（`/welcome`）

`/welcome <文章>` で本文を置き換えられます（最大 320 文字）。1 行目の「この部屋はホスト専用MOD「PocketRoles」を使用しています…」は MOD ポリシー上の案内なので **必ず付き、外せません**。

- `\n`（バックスラッシュと n の 2 文字）で改行。
- 差し込み: `{rules}` = ルール行、`{roles}` = 有効な役職の一覧、`{settings}` = 現在の設定（`/show` と同じ内容）、`{help}` = ヘルプと言語変更の 1 行、`{version}` = MOD のバージョン。
- `{settings}` を書かなくても「挨拶に設定を含める」がオンなら設定は末尾に付きます。`{rules}` を書かなくても、`/rules` でルールを設定していればルール行は末尾に付きます（標準の「ルールなし」文は `{rules}` を書いた場所にだけ入ります）。
...
- 新しい部屋ができるとホストのチャットに「切断後に自動で再ホストしました。新しい部屋コード: XXXXXX」と出ます。**部屋コードは変わる** ので、参加者に伝え直してください（`/announce` でコピーして案内部屋の名前を直します。[第 25 章](#25-便利ホスト登録オフと案内部屋)）。ロビーの時間切れ対策には廃村（[第 16 章](#16-ロビーの残り時間自動開始廃村)）を使うと、コードを変えずに済みます。
...
- 登録済み（+25）の部屋は公開にしても一覧に **表示されません**（[3.2](#32-登録した部屋は公開部屋一覧に表示されません)）。一覧に出したい時は登録オフの「便利ホスト」部屋（役職なし）と案内部屋を使ってください（[第 25 章](#25-便利ホスト登録オフと案内部屋)）。
...
| キー（既定） | 動作 | 押し方 |
|---|---|---|
| **F7** | ロビーでは廃村（ロビー更新）、試合中はその試合を即終了して同じ部屋に戻る（[16.4](#164-廃村haison廃村f7-2)） | **3 秒以内に 2 回**。1 回目で画面上部に「F7: もう一度押すと実行します（廃村（ロビーを更新））」と出ます |
...
4. 試したい役職を指定します。名前でもプレイヤー番号（`/assign show` や色順の番号）でも指定できます。名前は部分一致でも通ります（1 人に絞れる場合）。
   ...
   役職は通常の抽選枠に関係なく付けられます。インポスター枠の役職（ヴァンパイア・マフィア）をクルーに付けると、その人はインポスターとして開始します。指定は 1 回の試合で消費されます（次の試合にはもう一度指定が必要）。
...
6. スマホの画面で確認します: イントロ（シェリフなら「インポスター」と出ます）、名前の上の役職名、開始約 8 秒後のチャット、キルボタン（シェリフの誤射など）、`/cmd n` の返答、会議画面の名前、会議での役職説明、ベント・サボタージュの拒否など。
...
1. **タイトル画面のパネル**: PC のメインメニュー右側に PocketRoles のパネル（アイコン、`v0.4.0 / Among Us 2026.8.18`、GitHub の行）が出て、GitHub の行をクリックするとブラウザが開くこと。オンラインメニューを開くと隠れ、戻ると再表示されること。
2. **設定タブ**: ロビーのノート PC で「PocketRoles」が一番上、「バニラ設定」で 3 つのバニラボタンが展開すること。役職 / ロビー / チャット / 見た目 / ホスト支援 のページ切り替え、「?」にマウスを乗せると左の説明欄に説明が出ること。
3. **挨拶**: スマホの挨拶に「翻訳あり…」の行と「English: /cmd lang en ｜ 中文: … ｜ 日本語: …」の行があること。
...
| 画面左上の表示 | `役職部屋 ABCDEF` | `便利ホスト ABCDEF`（`/move <コード>` 設定後は下に `役職→コード`） |

登録オフの部屋を作るとホストのチャットに「登録オフ（便利ホスト）の部屋なので役職なしのバニラで進行します。役職ありは登録(+25)の部屋で。」と出ます。切り替えは次に作る部屋から有効で、`/opt register on` で戻せます。

### 25.2 案内部屋（おすすめ: サブスマホ 1 台）

役職部屋は公開一覧に出ないので、**サブのスマホ（またはもう 1 台の PC）でバニラの公開部屋を作り、その名前とチャットに役職部屋のコードを書いておく** 方法で人を呼びます。参加者は案内部屋でコードを見て、役職部屋に入り直します。
...
- ロビーの左上の大きなコード表示は `/code` で消せます（`[Guide] ShowCodeOverlay`、設定タブ「部屋コードを大きく表示」）。設定画面（ロビーのパソコン）を開いている間と試合中は自動で隠れます。
...
- `[Guide] AutoRecreateRegistered = true`（設定タブ「/move 後に登録部屋へ作り直す」、`/opt guide.autoreg on`）にすると、`/move` の 30 秒後に **この部屋自体を登録オンで作り直します**（「30秒後にこの部屋を役職ありの部屋として作り直します。新しいコードで入り直してください」→ 5 秒前に再通知 → 作り直し）。新しいコードはホストのチャットに出るので、`/announce` で案内部屋に書き写してください。既定はオフ（案内だけ）。
...
| 役職 | イントロの表示 |
|---|---|
| シェリフ、ジャッカル | **「インポスター」**（赤い画面、仲間は自分だけ）。本人のクライアントがインポスターとして動作するためです。本当の役職はイントロ直後の名前タグとチャットで伝わります |
| ヴァンパイア、マフィア | 通常のインポスターのイントロ（他のインポスターも表示） |
| マッドメイト、ジェスター、オポチュニスト、テロリスト、メイヤー、スニッチ、ライター、スピードブースター、ベイト | 通常の「クルー」のイントロ |
| 通常のインポスター | 通常のイントロ。ただしシェリフ・ジャッカルは仲間として表示されず、クルーに見えます |
...
| 役職 | タスクの見え方 |
|---|---|
| シェリフ、ジャッカル | クライアント上はインポスターなので **偽タスク**（インポスターと同じ表示）。タスクのコンソールは使えません。クルーのタスク進行には数えません |
| ジェスター、オポチュニスト、マッドメイト | 普通にタスクが表示され、操作もできますが、クルーのタスク進行には **数えません**（本人にはその区別は見えません。説明文に「タスクは偽物」と書いてあります） |
...
- シェリフ（ベント不可、サボ不可）、ジャッカル（サボ不可。ベントは設定）、マッドメイト（バニラのクルーと同じ）: クライアント上でインポスターのシェリフ・ジャッカルにはベントとサボタージュのボタン・マップが **表示されます** が、使うとホストが拒否します。ベントに入ると 0.5 秒後に追い出され、サボタージュとドア閉めは無視されます。「この役職はベントを使えません。」「この役職はサボタージュを使えません。」と 10 秒に 1 回チャットが届きます。
- ドアを **開ける** 操作は誰でもできます。
...
| マッドメイト | インポスター（ヴァンパイア・マフィア含む）の名前が赤 |
| インポスター（`KnownToImpostors = true` の時） | マッドメイトの名前の前に赤い **Ⓜ** |
...
- 届くタイミング: 参加 3 秒後（挨拶）、試合開始約 8 秒後（役職名と説明。通常役職には「あなたは通常のクルー/インポスターです。この試合には特殊役職がいます。」）、会議開始 1 秒後（役職説明の再送）、コマンドの返答、キル・ベント・サボタージュに関する通知、ロビーの残り時間・自動開始・延長・廃村・会議終了・ゲームマスターの案内。
...
- ホストの画面には実際の結果（「ジェスター勝利」「ジャッカル勝利」など）が陣営の色で表示されます。
...
- シェリフとジャッカルのイントロ画面は「インポスター」（仲間は自分だけ）と表示されます（[第 26 章](#26-バニラのプレイヤーにはどう見えるか)）。シェリフ・ジャッカルからは **他の全員がクルーに見えます**。ジャッカルが複数いてもお互いを認識できません。本当の役職はイントロの直後に名前タグとチャットで届きます（参加者には「イントロは当てにならない」と伝えておくと親切です）。
...
- 家庭用機・Switch・スマホのクイックチャット限定のプレイヤーは、役職通知やコマンドの返答を **読むことはできますが、コマンドを打てません**（`/lang` も使えないので、そういう参加者が多い時は部屋の既定言語を合わせてください）。
...
- 役職は「通常のクルー」「通常のインポスター」の枠からしか配られません。バニラの特殊役職（サイエンティスト、エンジニア、シェイプシフター等）になったプレイヤーには追加役職は付きません。ヴァンパイアとマフィアはインポスター枠を使うので、インポスター 1 人の設定でヴァンパイアを 1 にすると、その 1 人がヴァンパイアになります（`/assign` はこの制限を受けません）。
- シェリフ・ジャッカル・ライター・スピードブースターの効果はその人だけの「ゲーム設定」として送っているため、ロビー設定の変更などでバニラの設定が再送された直後は一時的に通常の値になることがあります（試合開始時、イントロ終了時、会議後に再送します）。
- テストモード中は勝敗判定が止まります。本番の前に必ず `/test off`（部屋を作り直せば自動で解除）。
- 自動再ホスト・`/move`・高 PING の作り直しで作り直した部屋は **ルームコードが変わります**（廃村なら変わりません）。案内部屋の名前を直してください。
...
2. 出てきたダイアログの **「メールを開く (不具合)」**（または「(要望)」）を押すと、宛先と件名（`[PocketRoles] 不具合報告 v0.4.0`）と本文のひな形（何が起きたか / いつ・部屋コード・人数 / 参加者からどう見えたか）が入った状態でメールソフトが開きます。**zip はデスクトップから添付してください**（メールソフトが無い場合は「zip の場所を開く」でファイルを確認し、ブラウザの Gmail などから送って構いません）。
...
- 動作確認: MOD 用コピーの `Among Us.exe` を起動し、`BepInEx\LogOutput.log` に `PocketRoles v0.4.0 loaded` が出ていること、Harmony のパッチエラーが無いことを確認します。

配布用 zip の作成: `powershell -NoProfile -ExecutionPolicy Bypass -File build-release.ps1`（`-SkipBuild` でビルドを省略）。`dist\PocketRoles-<ver>.zip`（`BepInEx\plugins\PocketRoles.dll`、`BepInEx\PocketRoles\lang\*.json`、README 3 種、LICENSE）と `dist\PocketRoles-Setup-<ver>.zip`（`PocketRolesLauncher.ps1`、`PocketRoles Launcher.cmd`、`assets\PocketRoles.ico`、`はじめに.txt`）、`SHA256SUMS.txt` ができます。バージョンは `PocketRoles.csproj` の `<Version>` から取ります。この 2 つを GitHub Releases に添付すると、ランチャーの「更新を確認」「インストール」が最新版を取得できます（リリースの資産名 `PocketRoles-<ver>.zip` を探します）。
...
このプロジェクトは **GNU General Public License v3.0 or later (GPL-3.0-or-later)** で公開します。全文: <https://www.gnu.org/licenses/gpl-3.0.html>（同梱の `LICENSE` も参照）。
