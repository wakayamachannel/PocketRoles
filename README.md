...
| 本体役職の秒数（v0.5.0）: バイタルの CD・表示時間、エンジニアのベント CD・ベント内時間、シェイプシフターの CD・変身時間、ファントムの CD・透明時間、守護天使の CD・守護時間、トラッカーの CD・遅延・追跡時間、ノイズメーカーの警報時間、ヴァイパーの溶解時間 | 本体の役職設定ページの範囲（例: バイタル表示 5〜30 秒） | **0〜600 秒**（矢印の刻みは本体のまま。`/vset` なら任意の値） | なし（固定。登録オフの部屋でも使えます: 公式サーバーはこの項目を検証しません） |
