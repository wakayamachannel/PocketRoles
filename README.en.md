...
- **Dleks (mirrored Skeld) is not available** (from v0.4.1 it is not offered in the map picker). In an unregistered lobby the server disconnected the host for "Hacking" right after Dleks was picked and the settings closed. A Hacking disconnect is not re-hosted automatically: re-create the lobby by hand.
